...
class EnhancedDataAgent:
    ...
    def _create_agent(self) -> AgentExecutor:
        ...
        # Use structured chat agent which properly handles tools and tool_names variables
...
def main():
    ...
    with st.sidebar:
        ...
        if llm_mode == "API LLM Mode":
            ...
            # --- Enhanced Model Fetching and Tool-Calling Validation ---
        ...
        # Enable analysis only after successful connection check and tool-calling validation
        tool_calling_valid = st.session_state.get("_api_model_tool_calling_valid", False)
        
        if llm_mode == "API LLM Mode":
            if api_connected and tool_calling_valid:
                apply_btn = st.button("향상된 데이터 분석 시작")
            elif not api_connected:
                st.info("먼저 API 연결 상태를 확인하세요.")
                apply_btn = False
            elif not tool_calling_valid:
                st.warning("⚠️ 선택된 모델이 Tool-calling을 지원하지 않습니다. 에이전트 기능이 제한될 수 있습니다.")
                apply_btn = st.button("향상된 데이터 분석 시작 (제한된 기능)")
            else:
            ...
            if local_connected:
                ...
                st.info("먼저 로컬 LLM 연결 상태를 확인하세요.")
                apply_btn = False
